#!/usr/bin/env python3
"""Route module for thr basic flask app API
"""
...
def users() -> str:
    ...
    Return the end-point to register a user
    """
...
def login():
    ...
    Return the request with form data with email and password fields
    """
...
def logout():
    ...
    Find the user with requested session ID, if exists, destroy session
    Redirect the user to GET /, if doesnt exists, respond with 403 HTTP
    status
    """
...
def get_reset_password_token_route() -> str:
    ...
    Return 403 status code if the email not registered
    Generate token and respond with the 200 HTTP status if exists
    """
    user_request = request.form
    user_email = user_request.get('email', '')
...
def update_password() -> str:
    ...
    Return a 403 HTTP code if the token is invalid
